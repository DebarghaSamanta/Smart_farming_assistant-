...
def send_judge_alert(alert, phone):
    """
    Send an alert generated by the judge demo.

    The judge demo has already decided whether an SMS is required.
    This function only handles delivery through TextBee.
    """

    if not alert.get("should_send_sms"):
        print("No judge SMS required.")
        return False

    message = alert.get("message")

    if not message:
        print("ERROR: Judge alert has no message.")
        return False
